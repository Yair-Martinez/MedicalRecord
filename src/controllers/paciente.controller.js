...
const createPaciente = async (req, res) => {
  try {
    ...
    const response = await pool.query(`INSERT INTO paciente (identificacion, email, password, telefono, status) VALUES ('${identificacion}', '${email}', '${passHash}', '${telefono}', '${status}');`);
...
const addData = async (req, res) => {
  try {
    ...
    if (!verify) {
      return res.status(403).json({ ok: false, message: "Sin Authorización" });
    }
